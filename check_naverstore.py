...
def run_diagnostics(html: str):
    ...
    if real_product_links:
        ...
        # li.Hz4XxKbt9h 카드 컨테이너 안의 모든 하위 요소를 "자기 자신만의 텍스트" 기준으로 나열
        # -> 상품명 / 가격 / 품절표시가 각각 어떤 class에 들어있는지 정확히 구분하기 위함
        card = sample.find_parent("li")
        if card:
            print(f"\n[진단] 카드 컨테이너(li) 내부 세부 요소 (class='{card.get('class')}')")
            for el in card.find_all(True):
                own_text = "".join(
                    s for s in el.find_all(string=True, recursive=False)
                ).strip()
                if own_text:
                    print(f"  <{el.name} class='{el.get('class')}'> 고유텍스트: '{own_text[:60]}'")
